...
async function repo() {
  ...
  const now = moment();
  const last_week = now.clone().subtract(7, "d"); // 7일 전 시점
...
function filterThisWeekDiscussion(repository, last_week, now) {
  return repository.discussions.edges.filter((edge) => {
    ...
    return (
      createdAt.isBetween(last_week, now, null, "[)") && // 7일 전부터 현재까지 포함
      edge.node.category.name !== "report" &&
      edge.node.category.name !== "announcements"
    );
  });
}
...
function makeContent(thisWeekDiscussionCount, sortedResult, now) {
  ...
  const maxCount = sortedResult[0][1]; // 가장 많이 푼 글 수
  const minCount = sortedResult[sortedResult.length - 1][1]; // 가장 적게 푼 글 수
